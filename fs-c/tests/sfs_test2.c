...
int difficult_test(void) {
  ...
  test_overflow_open(file_id, file_size, write_ptr, file_names, write_buf,
                     ABS_CAP_FD, &err_no);
  // So we have to open new files.
  test_open_new_files(file_names, file_id, num_file, &err_no);
  // Heavy write into the file system
  for (int i = 0; i < iterations; i++) {
    res = test_difficult_write_files(file_id, file_size, write_ptr, write_buf,
                                     num_file, &err_no);
    if (res < 0)
      break;
  }
  ...
  for (int i = 0; i < iterations; i++) {
    test_random_read_files(file_id, file_size, write_ptr, write_buf, num_file,
                           &err_no);
  }
  // Test out a few errors you should catch
  test_read_write_out_of_bound(file_id, file_size, write_buf, num_file,
                               &err_no);
  // Read All
  test_read_all_files(file_id, file_size, write_buf, num_file, &err_no);
  // Crank the file system to max for two files. Assumes we have two files at
  // least.
  ...
  test_remove_files(file_id, file_size, write_ptr, file_names, write_buf,
                    num_file, &err_no);
