...
int test_persistence(int *error, int write_length) {
  ...
  int error_num = 0;
  int temp;
  pid = fork(); // Split processes so that no memory structure would remain in
                // the main thread
  if (pid == 0) {
    for (int i = 0; i < 20; i++) {
      write_data[i] = rand_text(write_length);
    }
    pid = fork(); // Split again for the write so both read and write
                  // happens on different processes
    if (pid == 0) {
      ...
      for (int i = 0; i < 20; i++) {
        int32_t r = ssfs_fwrite(file_id, write_data[i], write_length);
        if (r != write_length) {
          printf("Expected, Actual = %d, %d\n", write_length, r);
      ...
      waitpid(pid, &temp, 0);
      if (WIFEXITED(temp) == 0) {
        printf("Errors detected in Write Section. Persistence test "
               "exiting ... Error code: %d\n139 is Segfault.\n. Check "
               "valgrind for more information\n",
               temp);
        error_num = 10;
        exit(error_num);
      } else {
    ...
    if (WIFEXITED(error_num) == 0) {
      printf("Errors detected while checking removed files.. "
             ...
             "Segfault. Check valgrind for more information\n",
             error_num);
      error_num = 10;
    } else {
...
/*
Plays around with frseek and fwseek. Will shift the read and write pointer back
by offset at the end if nothing fails.
...
int test_seek(int *file_id, int *file_size, int *write_ptr, char **write_buf,
  ...
  for (int i = 0; i < num_file; i++) {
    // Just testing the shift for beyond seek boundaries before actually
    // doing it.
    res = ssfs_frseek(file_id[i], -1);
    if (res >= 0)
      fprintf(stderr, "Warning: ssfs_frseek returned positive. Negative "
                      "seek location attempted. Potential frseek "
                      "fail?\n");
    res = ssfs_frseek(file_id[i], file_size[i] + 100);
    if (res >= 0)
      fprintf(stderr, "Warning: ssfs_frseek returned positive. Seek "
                      "location beyond file size attempted. Potential "
                      "frseek fail?\n");
    res = ssfs_fwseek(file_id[i], -1);
    if (res >= 0)
      fprintf(stderr, "Warning: ssfs_frseek returned positive. Negative "
                      "seek location attempted. Potential fwseek "
                      "fail?\n");
    ...
    if (res < 0)
      fprintf(stderr, "Warning: ssfs_fwseek returned negative. Potential "
                      "fwseek fail?\n");
    write_ptr[i] -= offset;
    if (write_ptr[i] < 0)
      write_ptr[i] = 0;
...
/*
Attempts to Reads all currently written text. If rseek fails, this will likely
fail as well.
*/
int test_read_all_files(int *file_id, int *file_size, char **write_buf,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    if (strcmp(buf, write_buf[i]) != 0) {
      ...
      *err_no += 1;
      printf("%d %d %d\n", (int32_t)strlen(buf), (int32_t)strlen(write_buf[i]),
             (int32_t)file_size[i]);
    }
  }
  free(buf);
...
int test_simple_read_files(int *file_id, int *file_size, char **write_buf,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    res = ssfs_fread(file_id[i], buf, (int32_t)strlen(test_str));
    buf[strlen(test_str)] = '\0';
    if (res != (int32_t)strlen(test_str))
      fprintf(stderr, "Warning: ssfs_fread should return number of bytes "
                      "read. Potential read fail?\n");
...
int test_simple_write_files(int *file_id, int *file_size, int *write_ptr,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    write_buf[i][file_size[i]] = '\0';
    res = ssfs_fwrite(file_id[i], test_str, (int32_t)strlen(test_str));
    if (res != (int32_t)strlen(test_str))
      fprintf(stderr, "Warning: ssfs_fwrite should return number of "
                      "bytes written. Potential write fail?\n");
  }
  return 0;
}
...
int test_difficult_read_files(int *file_id, int *file_size, int *write_ptr,
  ...
  UNUSED(file_size);
  int res = 0;
  char *buf = calloc((size_t)(read_length + 1), sizeof(char));
  char temp;
  ...
  if (strcmp(buf, write_buf[index] + (int)sizeof(char) * (write_ptr[index] -
                                                          read_length)) != 0) {
    fprintf(stderr, "Error: \nRead failed\n");
    *err_no += 1;
  }
...
int test_random_read_files(int *file_id, int *file_size, int *write_ptr,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    if (strcmp(buf, write_buf[i] + (int32_t)sizeof(char) * (start_index)) !=
        0) {
      fprintf(stderr, "Error: \nRead failed\n");
      *err_no += 1;
    }
...
int test_difficult_write_files(int *file_id, int *file_size, int *write_ptr,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    if (file_size[i] > 1)
      ...
      rand_offset = 0;
    // We want the size to mostly increase so we'll load the dice so to
    // speak.
    // More likely the file will increase instead of getting the write
    // pointer shifted a lot.
    ...
    if (write_ptr[i] + (int32_t)strlen(text) >= MAX_BYTES - 1) {
      free(text);
      printf("\n-------------------------------\nTest_num[%d]: Current "
             "Error Num: %d\n--------------------------------\n\n",
             test_num, *err_no);
    ...
    memcpy(write_buf[i] + (int32_t)sizeof(char) * write_ptr[i], text,
           strlen(text));
    ...
    if (res < 0)
      fprintf(stderr, "Warning: ssfs_frseek returned negative. Potential "
                      "frseek fail?\n");
    res = ssfs_fwrite(file_id[i], text, (int32_t)strlen(text));
    ...
    write_buf[i][file_size[i]] = '\0';
    if (res != (int32_t)strlen(text))
      fprintf(stderr, "Warning: ssfs_fwrite should return number of "
                      "bytes written. Potential write fail?\n");
    // Read test for if we wrote in properly
    test_difficult_read_files(file_id, file_size, write_ptr, write_buf, i,
                              (int32_t)strlen(text), err_no);
    free(text);
  }
  printf("\n-------------------------------\nTest_num[%d]: Current Error "
         "Num: %d\n--------------------------------\n\n",
...
int test_write_to_overflow(int *file_id, int *file_size, char **write_buf,
  ...
  char **buffer =
      calloc(ABS_CAP_FILE_SIZE / MAX_WRITE_BYTE + 1, sizeof(char *));
  int num_written = 0;
  int start_index;
  ...
  for (;;) {
    ...
    if ((size_t)(file_size[index]) + strlen(text) > ABS_CAP_FILE_SIZE) {
    ...
    res = ssfs_fwrite(file_id[index], text, (int32_t)strlen(text));
    if (res <= 0) {
      printf("Reached maximum file capacity. This is OK.\n Wrote %d bytes\n",
             file_size[index]);
      free(text);
      break;
    } else if (res != MAX_WRITE_BYTE) { // Else if the write isn't our file
                                        // length, it's an error
      fprintf(stderr,
              "Error: Invalid write. \nWrote %d when was supposed to be "
              "%d\n\n",
              res, MAX_WRITE_BYTE);
      *err_no += 1;
    }
...
int test_read_write_out_of_bound(int *file_id, int *file_sizes,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    if (res > 0) {
      fprintf(stderr,
              "Error: Invalid write length.\nWrote %d when was supposed "
              "to be %d\n\n",
              res, -1);
      *err_no += 1;
      break;
    ...
    if (res != file_sizes[i]) {
      fprintf(stderr,
              "Error: Length read should be file size\nRequested %d to "
              "read, Should be Read: %d, Read %d\n",
...
int test_overflow_open(int *file_id, int *file_sizes, int *write_ptr,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    // If we hit the cap and the file_id return is negative, we are good and
    // stop
    if (file_id[i] < 0) {
  ...
  for (int i = 0; i < ret; i++) {
    for (int j = i + 1; j < ret; j++) {
      if (file_id[i] == file_id[j]) {
        fprintf(stderr, "Warning: the file descriptors probably "
                        "shouldn't be the same?\n");
      }
    }
  }
  // Remove them all and do it again.
  test_remove_files(file_id, file_sizes, write_ptr, file_names, write_buf, ret,
                    err_no);
  ...
  // Returns the max number of files your file system can support or 1000 I
  // think.
  return ret;
}
...
int test_remove_files(int *file_id, int *file_size, int *write_ptr,
  ...
  for (int i = 0; i < num_file && i < ABS_CAP_FD; i++) {
    ...
    if (res < 0)
      fprintf(stderr, "Warning: ssfs_fclose returned negative value. "
                      "Potential fclose fail?\n");
    file_size[i] = 0;
    write_ptr[i] = 0;
    free(file_names[i]);
    file_names[i] = NULL;
...
Attemping to close using an ID that's beyond the currently assigned IDs by 1.
...
int test_close_files(char **file_names, int *file_id, int num_file,
  ...
  for (int i = 0; i < num_file; i++) {
    ...
    if (res >= 0) {
      fprintf(stderr,
              "Error: ssfs_fclose returned positive. Did it actually "
              "close the file %s?\n",
              file_names[i]);
      *err_no += 1;
    } else {
    ...
    if (res >= 0) {
      fprintf(stderr,
              "Error: ssfs_fclose returned positive. Attempted to close "
              "a file %s with file_id %d\n",
              file_names[i], -MAX_BYTES);
      *err_no += 1;
    } else {
      fprintf(stderr, "Failed to close. This is OK.\n");
    }
    // Finally, we end this with an attempt to open beyond the current file
    // limit.
    ...
    if (res > 0) {
      fprintf(stderr,
              "Error: ssfs_fread returned positive length. File %s "
              "should be closed\n",
              file_names[i]);
      *err_no += 1;
    } else {
...
Hence, it will not generate new names, but reutilize names in the file_names
array
Will give error if different files has same file descriptors.
*/
