...
super_block_t sb;
fbm_table_t fbm_table;
dir_entry_t dir_table[MAX_FILES];
inode_t inode_table[MAX_FILES];
file_entry_t file_entry_table[MAX_OPEN_FILES];
...
int32_t sb_read(super_block_t* sb) {
    ...
    memcpy(sb, mem, sizeof(super_block_t));

    return MY_OK;
}

int32_t sb_update(const super_block_t sb) {
    char mem[sb.blocks_size];
    memset(mem, 0, sizeof(mem));
    memcpy(mem, &sb, sizeof(sb));

    if(write_blocks(sb.sb_block_idx, sb.sb_block_num, mem) != sb.sb_block_num)
        return MY_ERR;
...
int32_t sb_init(super_block_t* sb) {
    ...
    sb->blocks_size = BLOCK_SIZE;
    sb->dir_block_idx = DIR_BLOCK;
    sb->dir_block_num = DIR_BLOCK_NUM;
    sb->fbm_block_idx = FBM_BLOCK;
    sb->fbm_block_num = FBM_BLOCK_NUM;
    sb->inode_block_idx = INODE_BLOCK;
    sb->inode_block_num = INODE_BLOCK_NUM;
    sb->sb_block_idx = SB_BLOCK;
    sb->sb_block_num = SB_BLOCK_NUM;
...
int32_t fbm_read(fbm_table_t* fbm_table) {
    ...
    if(read_blocks(sb.fbm_block_idx, sb.fbm_block_num, mem) != sb.fbm_block_num)
        return MY_ERR;

    memcpy(fbm_table, mem, sizeof(fbm_table_t));
...
int32_t fbm_update(const fbm_table_t fbm_table) {
    char mem[sb.blocks_size];
    memset(mem, 0, sizeof(mem));
    memcpy(mem, &fbm_table, sizeof(fbm_table));
...
int32_t block_allocate(fbm_table_t* fbm_table, int32_t idx) {
    ...
    if(idx >= 0 && idx < (int32_t)sb.blocks &&
       fbm_table->block[idx] == ENTRY_FREE) {
        fbm_table->block[idx] = ENTRY_TAKEN;
        if(fbm_update(*fbm_table) == MY_ERR)
            return MY_ERR;
    ...
    int32_t r = 0;
    for(size_t i = 0; i < sb.blocks; i++) {
        if(fbm_table->block[i] == ENTRY_FREE) {
            fbm_table->block[i] = ENTRY_TAKEN;
            r = i;
            break;
        }
    }
...
int32_t block_deallocate(fbm_table_t* fbm_table, int32_t idx) {
    if(fbm_table == NULL || idx < 0)
        return MY_ERR;

    if(idx < (int32_t)sb.blocks) {
        if(fbm_table->block[idx] == ENTRY_TAKEN) {
            fbm_table->block[idx] = ENTRY_FREE;
...
int32_t dir_find(dir_entry_t* d, uint32_t size, const char* name) {
    ...
    for(size_t i = 0; i < size; i++) {
        if(d[i].free == ENTRY_TAKEN &&
           strncmp(d[i].fn, name, MAX_FN_LEN) == 0) {
            r = i;
            break;
        }
    }
...
int32_t
dir_add(dir_entry_t* d, uint32_t size, const char* name, uint32_t node) {
    assert(size == MAX_FILES);
...
int32_t inode_read(inode_t* p, uint32_t size) {
    ...
    if(p == NULL ||
       read_blocks(sb.inode_block_idx, sb.inode_block_num, p) !=
           sb.inode_block_num)
        return MY_ERR;
...
int32_t inode_update(const inode_t* p, uint32_t size) {
    ...
    if(p == NULL ||
       write_blocks(sb.inode_block_idx, sb.inode_block_num, p) !=
           sb.inode_block_num)
        return MY_ERR;
...
int32_t* inode_get_block_list(const inode_t p, uint32_t* size) {
    ...
    *size = MAX_BLOCKS_PER_FILE;
    ...
    char* buf = malloc(sb.blocks_size);
    int32_t* iptr = (int32_t*)&buf[0];

    assert(p.next != ENTRY_INVALID);
    assert(read_blocks(p.next, 1, buf) == 1);
...
void mkssfs(int fresh) {
    if(fresh) {
        ...
        for(int32_t i = sb.dir_block_idx;
            i < sb.dir_block_idx + sb.dir_block_num;
            i++)
            assert(block_allocate(&fbm_table, i) == i);

        for(int32_t i = sb.inode_block_idx;
            i < sb.inode_block_idx + sb.inode_block_num;
            i++)
            assert(block_allocate(&fbm_table, i) == i);
...
int ssfs_fopen(char* name) {
    ...
    if(dir_idx == MY_ERR) {
        ...
        p->next = block_allocate(&fbm_table, -1);
        if(p->next == MY_ERR) {
            p->next = ENTRY_INVALID;
            return -1;
        }

        char buf[sb.blocks_size];
        int32_t* iptr = (int32_t*)&buf[0];
        ...
        assert(write_blocks(p->next, 1, buf) == 1);

        assert(inode_update(inode_table, MAX_FILES) == MY_OK);
        assert(dir_add(dir_table, MAX_FILES, name, inode_idx) == MY_OK);
        assert(dir_update(dir_table, MAX_FILES) == MY_OK);

        return fdt_add(file_entry_table, MAX_OPEN_FILES, inode_idx);
    } else {
        ...
        int32_t fd = fdt_add(file_entry_table, MAX_OPEN_FILES, inode_idx);
        if(fd == MY_ERR)
            return -1;

        file_entry_table[fd].ptr_read = 0;
        file_entry_table[fd].ptr_write = inode_table[inode_idx].size;
...
int ssfs_fwrite(int fileID, char* buf, int length) {
    ...
    if(fileID >= 0 && fileID < MAX_OPEN_FILES) {
        ...
        int32_t cur_block = (fd->ptr_write + len) / sb.blocks_size;
        ...
        memcpy(&file_buf[fd->ptr_write], buf, len);
        fd->ptr_write += len;
        written_bytes = len;
...
int ssfs_fread(int fileID, char* buf, int length) {
    ...
    if(fileID >= 0 && fileID < MAX_OPEN_FILES) {
        ...
        int32_t cur_block = (fd->ptr_read + len) / sb.blocks_size;
        ...
        memcpy(buf, &file_buf[fd->ptr_read], len);
        fd->ptr_read += len;
        read_bytes = len;
