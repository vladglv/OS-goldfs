...
int difficult_test() {
    ...
    for(int i = 0; i < iterations; i++) {
        res = test_difficult_write_files(
            file_id, file_size, write_ptr, write_buf, num_file, &err_no);
        if(res < 0)
            break;
    }
    // //More heavy random access memory reads
    for(int i = 0; i < iterations; i++) {
        test_random_read_files(
            file_id, file_size, write_ptr, write_buf, num_file, &err_no);
    }
    // Test out a few errors you should catch
    test_read_write_out_of_bound(
        file_id, file_size, write_buf, num_file, &err_no);
    // Read All
    ...
    for(int i = 0; i < iterations; i++) {
        ...
        test_seek(
            file_id, file_size, write_ptr, write_buf, num_file, 10, &err_no);
    }
    // More heavy random access memory reads
    for(int i = 0; i < iterations; i++) {
        test_random_read_files(
