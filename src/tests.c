    ...
    if(pid == 0) {
        ...
        if(pid == 0) {
            ...
            if(pid == 0) {
                ...
                for(int i = 0; i < 20; i++) {
                    if(ssfs_fread(file_id, read_buf, write_length) !=
                       write_length) {
                ...
                file_id = ssfs_fopen(file_name);
                ssfs_frseek(file_id, 0); // set seek to 0
                if(ssfs_fread(file_id, read_buf, 512) > 0 &&
                   strlen(read_buf) > 0) {
...
int test_seek(int* file_id,
    ...
    for(int i = 0; i < num_file; i++) {
        ...
        if(res >= 0)
            fprintf(stderr,
                    "Warning: ssfs_frseek returned positive. Negative "
                    "seek location attempted. Potential frseek "
                    "fail?\n");
        res = ssfs_frseek(file_id[i], file_size[i] + 100);
        if(res >= 0)
            fprintf(stderr,
                    "Warning: ssfs_frseek returned positive. Seek "
        ...
        if(res < 0)
            fprintf(stderr,
                    "Warning: ssfs_frseek returned negative. Potential "
                    "frseek fail?\n");
        res = ssfs_fwseek(file_id[i], file_size[i] - offset);
        if(res < 0)
            fprintf(stderr,
                    "Warning: ssfs_fwseek returned negative. Potential "
                    "fwseek fail?\n");
        write_ptr[i] -= offset;
        if(write_ptr[i] < 0)
            write_ptr[i] = 0;
...
int test_read_all_files(int* file_id,
    ...
    for(int i = 0; i < num_file; i++) {
        ...
        if(res != file_size[i])
            fprintf(stderr,
                    "Warning: ssfs_fread should return number of bytes "
...
int test_simple_write_files(int* file_id,
    ...
    for(int i = 0; i < num_file; i++) {
        ...
        if(res != (int32_t)strlen(test_str))
            fprintf(stderr,
                    "Warning: ssfs_fwrite should return number of "
...
int test_difficult_read_files(int* file_id,
    ...
    if(strcmp(buf,
              write_buf[index] +
                  sizeof(char) * (write_ptr[index] - read_length)) != 0) {
        fprintf(stderr, "Error: \nRead failed\n");
        *err_no += 1;
    }
...
int test_difficult_write_files(int* file_id,
    ...
    for(int i = 0; i < num_file; i++) {
        ...
        test_difficult_read_files(
            file_id, file_size, write_ptr, write_buf, i, strlen(text), err_no);
        free(text);
    }
    printf("\n-------------------------------\nTest_num[%d]: Current Error "
           "Num: %d\n--------------------------------\n\n",
...
int test_write_to_overflow(int* file_id,
    ...
    for(;;) {
        ...
        if(res <= 0) {
            printf(
                "Reached maximum file capacity. This is OK.\n Wrote %d bytes\n",
                file_size[index]);
            free(text);
            break;
    ...
    for(int i = 0; i < num_written; i++) {
        ...
        if(ssfs_fread(file_id[index], read_buffer, read_length) < 0) {
            ...
            *err_no += 1;
            ...
            fprintf(stderr,
                    "Error: Read length error. Expected %d but received %d\n",
                    read_length,
                    (int32_t)strlen(read_buffer));
            *err_no += 1;
    ...
    for(int i = 0; i < num_file; i++) {
        ...
        if(res != file_sizes[i]) {
            fprintf(stderr,
                    ...
                    ABS_CAP_FILE_SIZE + file_sizes[i],
                    file_sizes[i],
                    res);
            *err_no += 1;
        }
...
int test_overflow_open(int* file_id,
    ...
    for(int i = 0; i < ret; i++) {
        for(int j = i + 1; j < ret; j++) {
            if(file_id[i] == file_id[j]) {
                fprintf(stderr,
                        "Warning: the file descriptors probably "
    ...
    test_remove_files(
        file_id, file_sizes, write_ptr, file_names, write_buf, ret, err_no);
...
int test_remove_files(int* file_id,
    ...
    for(int i = 0; i < num_file && i < ABS_CAP_FD; i++) {
        ...
        if(res < 0)
            fprintf(stderr,
                    "Warning: ssfs_fclose returned negative value. "
...
*/
int test_open_new_files(char** file_names,
                        int* file_id,
                        int num_file,
                        int* err_no) {
    if(num_file < 1)
...
Will give error if different files has same file descriptors.
*/
int test_open_old_files(char** file_names,
                        int* file_id,
                        int num_file,
                        int* err_no) {
    if(num_file < 1)
